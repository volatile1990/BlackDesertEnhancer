...
public class AccessoryProfitCalculator {
    ...
    public void calculateAndPrintProfits() {
        ...
        for (Accessory accessory : accessories) {
            ...
            results.add(new AccessoryResult(accessory.getName(), accessory.getBaseStock(), triResult.avgItems, triProfit, tetResult.avgItems, tetProfit));
        }
    ...
    private void printProfitTable(List<AccessoryResult> results, String title, boolean isTri) {
        String format = "%-39s | %13s | %12s | %15s | %15s | %15s";
        log.info("\n{}", title);
        log.info("=".repeat(125));
        log.info("{}", String.format(format, "Name", "Avg " + (isTri ? "TRI" : "TET") + " Items", "Base Stock", (isTri ? "TRI" : "TET") + " Profit", "TRI Profit/Item", "TET Profit/Item"));
        log.info("-".repeat(125));
        ...
        for (AccessoryResult result : results) {
            ...
            log.info("{}", String.format(format, truncateString(result.name, 39), items, result.baseStock, formatNumber(profit), formatNumber(result.triProfitPerItem), formatNumber(result.tetProfitPerItem)));
        }
        log.info("=".repeat(125));
    }
